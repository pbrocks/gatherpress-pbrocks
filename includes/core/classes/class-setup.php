...
class Setup
{
    ...
    public function check_users_can_register(): void
    {
        if (filter_var(get_option('users_can_register'), FILTER_VALIDATE_BOOLEAN) 
            ...
            || filter_var(! current_user_can('manage_options'), FILTER_VALIDATE_BOOLEAN) || (       false === strpos(get_current_screen()->id, 'gatherpress') 
        ...
        if ('gatherpress_suppress_site_notification' === filter_input(INPUT_GET, 'action') 
    ...
    public function check_gatherpress_alpha(): void
    {
        if (defined('GATHERPRESS_ALPHA_VERSION') 
            || filter_var(! current_user_can('install_plugins'), FILTER_VALIDATE_BOOLEAN) || (       false === strpos(get_current_screen()->id, 'plugins') 
